...
function RegistrationForm(props) {
  ...
  //   return false
  // }
  // pageBlockers={[pageBlocker(['name', 'email', 'password'])]}
  return (
    <RegisterContext.Provider value={{ state, dispatch }}>
      <div className="p-3">
        <Stepper preventSkip={false} onSubmit={console.log}>
          <FormUsername label="user" />
          <FormOrganization label="organization" />
          <FormOrganizationContact label="contact" />
        </Stepper>
      </div>
...
function FormUsername() {
  ...
  const handleChange = key => e => {
    ...
    dispatch({ type: 'blocked', payload: { [key]: '' } })
  }
  return (
    <div>
      <Input
...
function FormOrganizationContact() {
  return (
    <div>
      <Input id="company.contact.name" type="text" label="Contact's name" placeholder="person to contact when needed" />
